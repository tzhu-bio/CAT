...
#' @param prefix  The prefix of save files.
#'
#' @return
#' @export
#'
...
GWASEnrichment <- function(euclideanNorm_file, snp, trait, output_path){
  ...
  overlap_snp_peak <- function(snp, rank_data){
    ...
    if (length(overlap_idx) > 0) {
      overlap_snp <- snp_dt[GenomicRanges::queryHits(overlap_idx),]
      overlap_quant <- quant_dt[GenomicRanges::subjectHits(overlap_idx),]
...
#' Title   The function to integrate the GWAS enrichment results.
#'
