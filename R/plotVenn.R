...
#' @param colors Default is Tableau_10
#' @param opacity Default is 0.3
#' @param plotsize Default is 15
#'
...
#' @param input_df A dataframe contains key and value(From get_combain_result())
#' @param bed_list A list contains records of all bed files(Read by function get_beds())
#' @param decreasing Boolean value, default is T
#' @param angles The angle of mainbar text, default is 45
#' @param text_scale Default is 0.65
#' @param point_size Default is 2.8
#' @param line_size Default is 1
#'
...
#' @param pattern The suffix of the bed files, default is '.bed'
...
#'
#' @param bed_list A list contains records of all bed files(Read by function get_beds())
#' @param picture Reture plot object, default is True
#' @param method Method of corrplot, default is 'pie', can be "circle", "square", "ellipse", "number", "shade", "color", "pie"
...
calPairwise <- function(bed_list, picture=T, method='pie', color="Blues",  tl_col="black"){
  ...
  if(picture){
    plot <- corrplot::corrplot(result_mat, method=method, type='full', diag = FALSE, col.lim=c(0,1), col=corrplot::COL1(color),  tl.col= tl_col)
    return(plot)
  }else{
    return(result_mat)
  }
